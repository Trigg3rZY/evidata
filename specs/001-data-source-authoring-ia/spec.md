...
**Input**: User description: "Data Source authoring IA: split the stacked authoring page into General, Schema, Policy, Context, Corrections, and Members & Access sections; replace flat table and sensitive-column checkboxes with searchable grouped multicheck controls; keep publish readiness/actions visible; keep the existing authoring payload and metadata schema."
...
An Owner/Admin working with a large schema can find tables and sensitive columns by name and toggle them from compact grouped controls.
...
1. **Given** a schema with dozens of tables, **When** the Admin searches by table name, **Then** matching tables are visible and can be included/excluded without scanning the full list.
...
- Data Source has a small schema: searchable grouped controls still work, but the page does not feel heavier than the current checkbox list.
...
- **FR-004**: The Schema section MUST replace flat table and sensitive-column checkbox lists with compact searchable grouped multicheck controls.
...
- **Schema Selection State**: UI projection of existing `includedTables`.
...
- **SC-002**: On a schema with at least 50 tables, an Admin can find and toggle a named table through search.
...
- Implementation will be split into at least two PRs: sectioned layout (#176), then searchable grouped selector (#174).
