...
- **`RunResult`** becomes a union: `{ kind: 'answer'; answer: Answer; queryRuns } | { kind: 'message'; message: AssistantMessageRecord; queryRuns }` where `AssistantMessageRecord = { id; investigationId; text; sql?; createdAt }`. The runner returns the message branch immediately when it sees `kind: 'message'` (no gate/execute/validate — there is nothing to validate, and nothing was executed).
- **`InvestigationService.ask`** returns `{ investigationId; result: RunResult }`; on a message it calls `store.saveMessage(...)` (a new, minimal write — no version chain, no evidence, no queryRuns rows) instead of `saveAnswer`. Messages do **not** increment the Answer `version` sequence (spec 02 §6).
