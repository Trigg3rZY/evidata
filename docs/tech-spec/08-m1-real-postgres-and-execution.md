...
- **Login identifier**: a **username** (a unique, free-form login handle — NOT RFC-email-enforced), so self-hosted/internal accounts like `admin001` work without an email. As built it's stored in the `users` table (physical column legacy-named `email`; the ORM field + all code use `username`). A separate, optional notification email can be added later when email-based invite/reset features land (M2 onboarding uses invite tokens, not email). `displayName` is a non-unique label, not a credential.
...
- **Driver**: the `DrizzleMetadataStore` adapter (`10 §5`) is reused; only the client changes. **As built (M1.8): the default self-hosted host is file-backed pglite** (`METADATA_DATA_DIR` — persists across restarts, no separate database to provision), with schema application made idempotent through the inlined migration journal. A real Postgres host (`METADATA_DATABASE_URL` + a node-postgres client + deploy-time `db:migrate`) is the opt-in host; runtime connects only after migrations are applied. Both satisfy metadata isolation (the pglite file / the separate DB is never a business Connection).
